...
resource "aws_route53_record" "k8s-api" {
  ...
  ttl = "300"
  records = [
    "${aws_instance.tungsten-k8s-master-server-1.public_ip}"
  ]
}

resource "aws_route53_record" "k8s-routes" {
  zone_id = "${aws_route53_zone.primary.zone_id}"
  name = "*.project-tungsten.com"
  type = "A"
  ttl = "300"
  records = [
    "${aws_instance.tungsten-k8s-worker-server-1.public_ip}",
    "${aws_instance.tungsten-k8s-worker-server-2.public_ip}"
  ]
}
